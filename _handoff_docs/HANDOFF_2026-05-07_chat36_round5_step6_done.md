# HANDOFF — page21 annotation cockpit MIGRATED; 6 of 22 pages done
...
compatibility — same single-file refactor pattern as page17 (round 5
step 5) and page18 (round 5 step 4).

```
atlases/inversion/pages/catalogue/
├── page21.js                       776 LOC ← refactored in-place
└── page21/
    └── _state.js                    16 LOC ← _pageState + setter (page21's own)
```
...
### Step 1 — page21.js refactored in-place

Same single-file pattern as page17/page18:
...
- Renamed verbatim `function refreshAnnotationCockpit()` →
  ...
  body, matching page17/18 convention).
...
page17/page18, page21's body was already written to access state through
...
### Step 2 — page21/_state.js (NEW, 16 LOC)

Same shape as page17/_state.js and page18/_state.js.

### Step 3 — Tests

- `tests/test_catalogue_page21.js`: replaced (was a stale chat-33 test
  that imported from the wrong path `../inversion_catalogue/page21.js`
...
- **page1/page2/page3/page17/page18 modules** — completely unchanged.
- **`shared/page1_data_helpers.js`** — unchanged. Page21 doesn't call
  `_esc` (the one shared helper that page17 picked up).
...
16, 16b, 19, page_overview, page_sv_evidence.
...
page in a row (after page18 round-5-step-4 and page17 round-5-step-5)
...
injection was needed — saving the AST-walking step from page17/18.
...
gracefully. Same approach as page17 with `_csGetSyntenyBlocks` /
`_csPermutationTest`. They'll land naturally with page2 / page16 /
page16b migration.
...
| **page16, page16b** | comparative | 2400+ each | multi-species cockpit; **would resolve `_csGetSyntenyBlocks`, `_csPermutationTest`, AND likely `computeTrackedLinkageProjection`** |
| **page8, 9, 15, 19, page_overview** | various | <105 each | tiny stubs; quick router-wiring rounds — could batch several in one round |
| **page4, 6, 7, 11** | review | 122-301 | review-stage pages |
| **page_sv_evidence** | review | 148 | SV evidence review |
| **page10** | catalogue | TBD | catalogue completion |
| **page5** | comparative | small (stub) | help-page stub |

Logical next priorities depending on goal:

- **Catalogue completion** — page9, page10, page_overview would round
  out the catalogue group. page_overview is the smallest, page10/9
  are medium.
- **Resolve cross-page runtime guards** — page2 (candidate focus deep
  dive) is already migrated, but the 3 helpers page21 stubs
  ...
  41196+ into `shared/`.
- **Quick wins / coverage** — tiny stubs (page8/9/15/19/page_overview)
