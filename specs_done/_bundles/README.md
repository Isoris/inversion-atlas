...
| `HOW_TO_USE_page_sv_evidence.md` | (standalone, authored 2026-05-15) | user guide for page_sv_evidence (SV evidence); thin-loader architecture (`window.AtlasSVEvidence` object with init/loadCandidate/destroy), exact fallback message when external module missing, lifecycle table with `__pageInitDone` + `__lastCid` double-guard, per-candidate JSON file location + schema highlights, all 6 pattern labels with rule summaries (full rules in SPEC §3.3), producer CLI example, 5 common gotchas, cross-refs to SPEC_sv_evidence_page.md. ALL FACTS VERIFIED against shipped code (page_sv_evidence.js lines 32-48, 120-191 + page_sv_evidence.html DOM ids + SPEC §3.3-3.4). | — |
