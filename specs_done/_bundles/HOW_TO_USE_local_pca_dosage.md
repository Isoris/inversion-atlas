# How to use local_pca_dosage — local PCA |Z|

**Page**: `local_pca_dosage` · stage `discovery` · label "local PCA \|Z\|"
...
```
atlases/inversion/
├── manifest.json                                ← local_pca_dosage entry
├── registries/data/pages.registry.json          ← local_pca_dosage _doc
├── pages/discovery/
│   ├── local_pca_dosage.html                               ← the HTML shell + DOM ids
│   ├── local_pca_dosage.js                                 ← main entry (mount/unmount/applyData)
│   └── local_pca_dosage/                                   ← 28 sub-modules
...
2. **Start the shell** (e.g. `python -m http.server` from the
   ...
   shows **local_pca_dosage — local PCA \|Z\|** as the first page.

3. **Pick a chromosome** in the topbar — the page reads
   `atlasState.shared.activeChrom`. Click the local_pca_dosage tab.
...
```
   ...
   with source='manual', confirmed=true
        ↓
   move to candidate_focus to deep-dive, karyotype_tier to karyotype-review,
...
See `local_pca_dosage/hotkeys.js` for the canonical list.
...
Reload the page — local_pca_dosage replays everything cached in IndexedDB:
...
- **candidate_focus** shows the per-candidate deep-dive (~15 sub-panels)
...
1. **Empty cluster cache.** If candidate_focus / karyotype_tier / boundary_refinement feel slow on
   first open, it's because local_pca_dosage's cluster-cache hasn't been
   warmed for this chromosome yet. Just open local_pca_dosage first; the
...
4. **Drag-drop doesn't work.** Check that you're dragging into the
   local_pca_dosage main area, not the topbar. The topbar's drop handler is
...
## What local_pca_dosage does NOT do
...
`docs/generated/page_contracts/local_pca_dosage/PAGE_CONTRACT.md` has the
