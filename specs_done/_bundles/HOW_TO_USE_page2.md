# How to use page2 — candidate focus

**Page**: `page2` · stage `discovery_2` · label "candidate focus"
...
This is the second page most users open after page1 — they pick a
candidate from page1's catalogue strip, then open page2 to
characterize it.
...
atlases/inversion/
├── pages/discovery/
│   ├── page2.html                          ← HTML shell + ~50 DOM ids
│   ├── page2.js                            ← main entry (4 orchestrators)
│   └── page2/
│       ├── _state.js                         ← _pageState (separate from page1's)
...
└── shared/
    ├── page1_data_helpers.js               ← cluster-cache + per-window dosage_chunks (shared with page1)
...
1. **"Page2 is slow on first open after chrom switch."** Page1
   hasn't been mounted yet → cluster-cache cold. Open page1 first
...
4. **"I confirmed a candidate but page1's lines don't show its
   band."** Refresh page1's lines panel by re-mounting (chrom switch
...
- **It does NOT compute live** — every panel reads either
  pre-computed layer data OR page1's cluster-cache. No live
...
- `SPEC_page1_candidate_mode_ui.md` (HANDOFF 2 detailed mode would
  ship a "detailed" tab on page2 alongside the current "default"
  tab — not yet wired)
- `SPEC_mendelian_inheritance_para_vs_peri_v1.md` (would add a
  Mendelian goodness-of-fit row to page2)

## Per-page contract

`docs/generated/page_contracts/page2/PAGE_CONTRACT.md`
