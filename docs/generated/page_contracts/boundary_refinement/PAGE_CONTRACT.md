# boundary_refinement — boundaries — Page Capability Contract
...
## Sub-modules in `boundary_refinement/`
...
- **REGISTRY MISMATCH (flagged step 22)**: declared
  ...
  `candidate_final_class` looks like karyotype_tier's territory;
  `candidate_breeding_card` does not appear anywhere in boundary_refinement
  source. **Swap hypothesis**: may have been swapped with karyotype_tier's
...
- **31 TODO_MISSING helpers** (`_bnd*` / `_wireRepeatDensity*`) —
  ...
  boundary_refinement-private (not fold-in opportunities).
...
- **Registry doc**: `pages.registry.json` → `pages.boundary_refinement._doc` (the
  fourth registry mismatch flag; swap hypothesis with karyotype_tier)
- **Handoffs**:
