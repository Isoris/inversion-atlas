# local_pca_dosage — local PCA |Z| — Page Capability Contract
...
- `theta_pi_*` layers (drive local_pca_theta_pi mirror — same panel architecture
  painted from the theta_pi envelope)
- `ghsl_panel` layer (drives local_pca_ghsl mirror — same panel architecture
...
See `local_pca_dosage/hotkeys.js` for the canonical hotkey list.
...
## Sub-modules in `local_pca_dosage/`
...
- **Registry doc**: `atlases/inversion/registries/data/pages.registry.json` → `pages.local_pca_dosage._doc`
- **Specs (done)**:
  - `specs_done/SPEC_band_track_extraction_and_l3_single_band_rows.md`
  - `specs_done/SPEC_l2_sweep_inheritance.md` (authored 2026-05-15 from `local_pca_dosage/l2_sweep.js`)
