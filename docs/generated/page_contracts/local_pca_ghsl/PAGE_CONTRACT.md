# local_pca_ghsl — local PCA GHSL — Page Capability Contract
...
Designed as the **third evidence axis** alongside local_pca_dosage (dosage) and
local_pca_theta_pi (θπ). A six-panel chromosome-wide scanner driven by **GHSL
...
- (When fully wired: scrubber cursor shared with local_pca_dosage.)
...
primitives as local_pca_theta_pi:
- `shared/per_l2_cluster.js`, `hungarian.js`, `contingency.js`,
  `kmeans.js`, `color_helpers.js`.

## Sub-modules in `local_pca_ghsl/`

| file | purpose |
|------|---------|
| `_state.js` | `_pageState` + setter — mirrors local_pca_dosage / local_pca_theta_pi lifecycle pattern |
...
- **Registry doc**: `atlases/inversion/registries/data/pages.registry.json` → `pages.local_pca_ghsl._doc`
- **Specs**: none directly
