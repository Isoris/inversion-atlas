...
//
// Sub-module + main re-export coverage for the page16b multi-species
// classification cockpit page (lifecycle scaffolding + verbatim
// chat-33 helper bodies via AST-shim injection).
//
// Round 5 step 20 (chat 39 cont., 2026-05-07): page16b promoted from
...
// _pageState live-binding. **Direct twin of page16 (step 11) in
...
// imported from `../inversion_comparative/page16b.js` (pre-migration
// path) and was a parse-check + content-scan stub.

import * as page16b from '../atlases/inversion/pages/comparative/page16b.js';
import * as state from '../atlases/inversion/pages/comparative/page16b/_state.js';
...
// -----------------------------------------------------------------------------
group('page16b.js: lifecycle entry-points');
check('exports mount',                          typeof page16b.mount === 'function');
check('exports unmount',                        typeof page16b.unmount === 'function');
check('exports renderMultiSpeciesPage',         typeof page16b.renderMultiSpeciesPage === 'function');
check('__MODULE_ID__ NOT exported',             !('__MODULE_ID__' in page16b));

// -----------------------------------------------------------------------------
group('page16b.js: 10 constants exported');
check('DOTPLOT_MASHMAP_TOOL',                   page16b.DOTPLOT_MASHMAP_TOOL === 'dotplot_mashmap_v1');
check('DOTPLOT_MASHMAP_LS_KEY',                 typeof page16b.DOTPLOT_MASHMAP_LS_KEY === 'string');
check('MULTI_SPECIES_UI_LS_KEY',                typeof page16b.MULTI_SPECIES_UI_LS_KEY === 'string');
check('SYNTENY_MULTISPECIES_LS_KEY',            typeof page16b.SYNTENY_MULTISPECIES_LS_KEY === 'string');
check('PHYLO_TREE_LS_KEY',                      typeof page16b.PHYLO_TREE_LS_KEY === 'string');
check('_MS_DEFAULT_SPECIES is array',           Array.isArray(page16b._MS_DEFAULT_SPECIES));
check('_MS_DEFAULT_SPECIES has Cgar',           page16b._MS_DEFAULT_SPECIES.some(sp => sp.id === 'Cgar' && sp.focal));
check('_MS_DEFAULT_SPECIES has Cmac',           page16b._MS_DEFAULT_SPECIES.some(sp => sp.id === 'Cmac' && sp.focal));
check('DXY_PER_INVERSION_LS_KEY',               typeof page16b.DXY_PER_INVERSION_LS_KEY === 'string');
check('CLASSIFICATIONS_LS_KEY',                 typeof page16b.CLASSIFICATIONS_LS_KEY === 'string');
check('TE_FRAGILITY_LS_KEY',                    typeof page16b.TE_FRAGILITY_LS_KEY === 'string');
check('KARYO_LINEAGE_LS_KEY',                   typeof page16b.KARYO_LINEAGE_LS_KEY === 'string');

// -----------------------------------------------------------------------------
group('page16b.js: 5 render entry points');
check('exports _renderMultiSpeciesPage',        typeof page16b._renderMultiSpeciesPage === 'function');
check('exports _msRenderActiveHeader',          typeof page16b._msRenderActiveHeader === 'function');
check('exports _msRenderTreeSvg',               typeof page16b._msRenderTreeSvg === 'function');
check('exports _msRenderCenterColumn',          typeof page16b._msRenderCenterColumn === 'function');
check('exports _msRenderDetailColumn',          typeof page16b._msRenderDetailColumn === 'function');

// -----------------------------------------------------------------------------
group('page16b.js: 30 IO loaders (6 layers × 5 functions)');
const layers = [
...
for (const [name, ...fns] of layers) {
  for (const fn of fns) {
    check(`${name}: exports ${fn}`, typeof page16b[fn] === 'function');
  }
}

// -----------------------------------------------------------------------------
group('page16b.js: 9 MS state / init / classification helpers');
const msStateFns = [
...
for (const fn of msStateFns) {
  check(`exports ${fn}`, typeof page16b[fn] === 'function');
}

// -----------------------------------------------------------------------------
group('page16b.js: 12 MS analysis helpers');
const msAnalysisFns = [
...
// -----------------------------------------------------------------------------
group('page16b.js: 8 karyotype helpers');
const karyoFns = [
...
const speciesList = page16b._msGetEffectiveSpeciesList();
check('_msGetEffectiveSpeciesList returns default when phyloTree is null',
      Array.isArray(speciesList) && speciesList.length === page16b._MS_DEFAULT_SPECIES.length);
check('returned list is a copy (different reference from _MS_DEFAULT_SPECIES)',
      speciesList !== page16b._MS_DEFAULT_SPECIES);
...
try { page16b.renderMultiSpeciesPage(synthState); }
