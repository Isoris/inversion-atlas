...
// smoke test for negative_regions (negative-regions catalogue — discovery stage).
//
...
//   - state slots: negativeRegions (array) + activeChrom (vs window_summary_table's
//     activeChrom + precomp).
...
const negative_regions = await import(`${WORKSPACE}/atlases/inversion/pages/discovery/negative_regions.js`);
const state  = await import(`${WORKSPACE}/atlases/inversion/pages/discovery/negative_regions/_state.js`);
...
// -----------------------------------------------------------------------------
// Minimal DOM polyfill — same shape as window_summary_table/9/17/18/21 smoke harnesses.
...
check('negative_regions has mount',                       typeof negative_regions.mount === 'function');
check('negative_regions has unmount',                     typeof negative_regions.unmount === 'function');
...
try { await negative_regions.mount(root, atlasState, registry); }
...
try { await negative_regions.mount(root, atlasState2, registry); }
...
try { await negative_regions.unmount(root); }
