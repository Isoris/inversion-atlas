...
// unmount lifecycle smoke test for karyotype_tier (karyotype/tier candidate-
// level review page). **First real review-stage migration**
// (review group: 3 of 5 → 4 of 5). Pattern 2 (single-file with
// _state.js) applied; closest peers are local_pca_theta_pi (step 10) and stats_profile
// (step 5) in pattern shape.
//
// karyotype_tier has 4 chat-33 functions + 1 page-private const (karyoState),
...
const karyotype_tier = await import(`${WORKSPACE}/atlases/inversion/pages/review/karyotype_tier.js`);
const state = await import(`${WORKSPACE}/atlases/inversion/pages/review/karyotype_tier/_state.js`);
...
// -----------------------------------------------------------------------------
// Minimal DOM polyfill — karyotype_tier uses getElementById, .style, .innerHTML,
...
// localStorage stub — setKaryoSubview persists, karyotype_tier module-load
...
check('karyotype_tier has mount',                     typeof karyotype_tier.mount === 'function');
check('karyotype_tier has unmount',                   typeof karyotype_tier.unmount === 'function');
check('karyotype_tier has refreshPage4 (wrapper)',    typeof karyotype_tier.refreshPage4 === 'function');
check('karyotype_tier has renderCandidateKaryotype',  typeof karyotype_tier.renderCandidateKaryotype === 'function');
check('karyotype_tier has renderCandidateTier',       typeof karyotype_tier.renderCandidateTier === 'function');
check('karyotype_tier has _refreshSubviewButtonStyles',typeof karyotype_tier._refreshSubviewButtonStyles === 'function');
check('karyotype_tier has setKaryoSubview',           typeof karyotype_tier.setKaryoSubview === 'function');
check('karyotype_tier has karyoState (const)',        typeof karyotype_tier.karyoState === 'object');
...
try { await karyotype_tier.mount(root, atlasState, registry); }
...
_resetNodes();
karyotype_tier.karyoState.subview = 'tier';
...
try { await karyotype_tier.mount(root, atlasState2, registry); }
...
try { karyotype_tier.setKaryoSubview('tier'); }
...
try { karyotype_tier.setKaryoSubview('karyotype'); }
...
check('setKaryoSubview("invalid") is a no-op (no throw, subview unchanged)',
      r3OK && karyotype_tier.karyoState.subview === 'karyotype');
...
try { karyotype_tier.refreshPage4(reRenderState); }
...
try { await karyotype_tier.unmount(root); }
