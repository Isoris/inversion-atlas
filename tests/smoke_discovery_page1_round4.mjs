...
//
// Smoke test for the round-4 split of page1 (chat 35, 2026-05-06).
...
const page1 = await import(`${WORKSPACE}/atlases/inversion/pages/discovery/page1.js`);
...
// page1.html identifies the canvases the draw functions hit. Provide them.
const ROOT = new FakeElement('div', 'page1-root');
...
for (const [name, fn] of [
  ['drawSim',          () => page1.drawSim(state)],
  ['drawSimMini',      () => page1.drawSimMini(state)],
  ['drawZ',            () => page1.drawZ(state)],
  ['drawLinesPanel',   () => page1.drawLinesPanel(state)],
  ['drawPCA',          () => page1.drawPCA(state)],
  ['drawAnchorStrip',  () => page1.drawAnchorStrip(state)],
  ['drawTracks',       () => page1.drawTracks(state)],
  ['updateWinLabel',   () => page1.updateWinLabel(state)],
]) {
...
try {
  await page1.mount(MOUNT_ROOT, fakeAtlasState, fakeRegistry);
  check('mount() ran end-to-end without throwing', true);
} catch (e) {
  check('mount() ran end-to-end without throwing', false, e.stack || e);
}
check('mount stashed page1 state on inversion bucket',
...
try {
  await page1.unmount(MOUNT_ROOT);
...
const stateMod = await import(`${WORKSPACE}/atlases/inversion/pages/discovery/page1/_state.js`);
const stateA = { tag: 'A' };
const stateB = { tag: 'B' };
