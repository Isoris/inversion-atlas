...
// unmount lifecycle smoke test for page16b (multi-species
// classification cockpit — comparative stage). **Direct twin of
// page16's smoke** (shipped step 11); third (and final) migrated
// comparative-stage page. Closes the comparative group.
//
// page16b is a verbatim-body page with 39 of 64 helpers shimmed via
...
const page16b = await import(`${WORKSPACE}/atlases/inversion/pages/comparative/page16b.js`);
const state   = await import(`${WORKSPACE}/atlases/inversion/pages/comparative/page16b/_state.js`);
...
// -----------------------------------------------------------------------------
// Minimal DOM polyfill — page16b uses getElementById / querySelector / appendChild /
...
check('page16b has mount',                 typeof page16b.mount === 'function');
check('page16b has unmount',               typeof page16b.unmount === 'function');
check('page16b has renderMultiSpeciesPage',typeof page16b.renderMultiSpeciesPage === 'function');
check('page16b has _renderMultiSpeciesPage',typeof page16b._renderMultiSpeciesPage === 'function');
check('page16b has _msInitState',          typeof page16b._msInitState === 'function');
check('page16b has _msGetActiveBreakpoint',typeof page16b._msGetActiveBreakpoint === 'function');
...
try { await page16b.mount(root, atlasState, registry); }
...
// _buildLegacyState defaults all 14 page16b-relevant slots
...
try { await page16b.mount(root, atlasState2, registry); }
...
const activeBp = page16b._msGetActiveBreakpoint();
...
// an array.)
const speciesListPopulated = page16b._msGetEffectiveSpeciesList();
...
const cls = page16b._msGetClassification('BP_LG12_001');
check('_msGetClassification("BP_LG12_001") returns architecture "A" via shim',
      cls && cls.architecture === 'A');

const noCls = page16b._msGetClassification('BP_NONEXISTENT');
...
try { page16b.renderMultiSpeciesPage(reRenderState); }
...
try { await page16b.unmount(root); }
