...
// lifecycle smoke test for annotation_cockpit (annotation cockpit).
//
...
const annotation_cockpit = await import(`${WORKSPACE}/atlases/inversion/pages/catalogue/annotation_cockpit.js`);
const state  = await import(`${WORKSPACE}/atlases/inversion/pages/catalogue/annotation_cockpit/_state.js`);
...
check('annotation_cockpit has mount',                       typeof annotation_cockpit.mount === 'function');
check('annotation_cockpit has unmount',                     typeof annotation_cockpit.unmount === 'function');
check('annotation_cockpit has refreshAnnotationCockpit',    typeof annotation_cockpit.refreshAnnotationCockpit === 'function');
...
try { await annotation_cockpit.mount(root, atlasState, registry); }
...
try { await annotation_cockpit.mount(root, atlasState2, registry); }
...
try { annotation_cockpit.refreshAnnotationCockpit(stashedState); }
...
const extent = annotation_cockpit._annoCockpitChromExtent([]);
...
try { await annotation_cockpit.unmount(root); }
