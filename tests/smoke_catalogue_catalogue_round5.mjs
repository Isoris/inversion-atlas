...
// lifecycle smoke test for the catalogue catalogue split.
//
...
// smoke_discovery_page2_round5.mjs). Only the pieces catalogue's mount path
...
check('catalogue has mount',                    typeof catalogue.mount === 'function');
check('catalogue has unmount',                  typeof catalogue.unmount === 'function');
check('catalogue has renderCataloguePage',      typeof catalogue.renderCataloguePage === 'function');
check('catalogue has initCataloguePage',        typeof catalogue.initCataloguePage === 'function');
...
try { await catalogue.mount(root, atlasState, registry); }
...
try { catalogue.renderCataloguePage(stashedState); }
...
try { catalogue.initCataloguePage(stashedState); }
...
try { await catalogue.unmount(root); }
