...
// lifecycle smoke test for local_pca_theta_pi (local-PCA-θπ chromosome-wide
// diversity scanner — the θπ sister of local_pca_dosage).
//
// Page12 is a discovery-stage page: same six-panel layout as local_pca_dosage, but
...
const local_pca_theta_pi = await import(`${WORKSPACE}/atlases/inversion/pages/discovery/local_pca_theta_pi.js`);
const state  = await import(`${WORKSPACE}/atlases/inversion/pages/discovery/local_pca_theta_pi/_state.js`);
...
// -----------------------------------------------------------------------------
// Minimal DOM polyfill — same shape as local_pca_dosage/2/3/9/10/17/18/21 smoke harnesses.
// Adds canvas-context shim because local_pca_theta_pi's draw paths use canvas extensively
...
check('local_pca_theta_pi has mount',                       typeof local_pca_theta_pi.mount === 'function');
check('local_pca_theta_pi has unmount',                     typeof local_pca_theta_pi.unmount === 'function');
check('local_pca_theta_pi has renderPage12',                typeof local_pca_theta_pi.renderPage12 === 'function');
check('local_pca_theta_pi has state-aware wrappers (8)',
      typeof local_pca_theta_pi.refreshThetaPiLayerStatus === 'function'
      && typeof local_pca_theta_pi.refreshThetaPiPanelVisibility === 'function'
      && typeof local_pca_theta_pi.drawThCusumHero === 'function'
      && typeof local_pca_theta_pi.drawThLinesPanel === 'function'
      && typeof local_pca_theta_pi.drawThSimMatPanel === 'function'
      && typeof local_pca_theta_pi.drawThZPanel === 'function'
      && typeof local_pca_theta_pi.drawThAnchorStripPanel === 'function'
      && typeof local_pca_theta_pi.drawThPcaPanel === 'function');
check('local_pca_theta_pi has verbatim helpers (8)',
      typeof local_pca_theta_pi._refreshThetaPiLayerStatus === 'function'
      && typeof local_pca_theta_pi._drawThPcaPanel === 'function');
...
try { await local_pca_theta_pi.mount(root, atlasState, registry); }
...
try { await local_pca_theta_pi.mount(root, atlasState2, registry); }
...
try { local_pca_theta_pi.renderPage12(stashedState); }
...
try {
  local_pca_theta_pi.refreshThetaPiLayerStatus(stashedState);
  local_pca_theta_pi.refreshThetaPiPanelVisibility(stashedState);
  local_pca_theta_pi.drawThCusumHero(stashedState);
}
...
try { await local_pca_theta_pi.unmount(root); }
