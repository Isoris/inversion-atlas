...
// lifecycle smoke test for local_pca_ghsl (GHSL haplotype-divergence page —
...
//     (this is the BEHAVIOURAL check that distinguishes local_pca_ghsl from
//     the pure-stub window_summary_table/negative_regions smokes)
...
const local_pca_ghsl = await import(`${WORKSPACE}/atlases/inversion/pages/discovery/local_pca_ghsl.js`);
const state  = await import(`${WORKSPACE}/atlases/inversion/pages/discovery/local_pca_ghsl/_state.js`);
...
// -----------------------------------------------------------------------------
// Minimal DOM polyfill — enriched for local_pca_ghsl: must support
...
// Register all 5 [data-gh-layer] nodes mirroring local_pca_ghsl.html.
const GH_LAYERS = [
  'ghsl_panel', 'ghsl_kstripes', 'ghsl_karyotype_runs',
  'ghsl_d17_envelopes', 'cusum_ghsl',
];
...
check('local_pca_ghsl has mount',                       typeof local_pca_ghsl.mount === 'function');
check('local_pca_ghsl has unmount',                     typeof local_pca_ghsl.unmount === 'function');
check('local_pca_ghsl has refreshGhslLayerStatus',      typeof local_pca_ghsl.refreshGhslLayerStatus === 'function');
check('local_pca_ghsl has _refreshGhslLayerStatus',     typeof local_pca_ghsl._refreshGhslLayerStatus === 'function');
...
try { await local_pca_ghsl.mount(root, atlasState, registry); }
...
try { await local_pca_ghsl.mount(root, atlasState2, registry); }
...
try { local_pca_ghsl.refreshGhslLayerStatus(reRenderState); }
...
try { await local_pca_ghsl.unmount(root); }
