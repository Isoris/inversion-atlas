...
// local_pca_theta_pi local-PCA-θπ chromosome-wide diversity scanner.
//
// Round 5 step 10 (chat 36, 2026-05-07): local_pca_theta_pi refactored from chat-33
...
//
// Page12 is the θπ sister of local_pca_dosage: same six-panel layout, but reads
// theta_pi_* layers from state.data instead of dosage. Empty-state
// placeholder until R-pipeline ships layers.

import * as local_pca_theta_pi from '../atlases/inversion/pages/discovery/local_pca_theta_pi.js';
import * as state  from '../atlases/inversion/pages/discovery/local_pca_theta_pi/_state.js';
...
// -----------------------------------------------------------------------------
group('local_pca_theta_pi.js: lifecycle entry-points (NEW round 5 step 10)');
check('exports mount',                   typeof local_pca_theta_pi.mount === 'function');
check('exports unmount',                 typeof local_pca_theta_pi.unmount === 'function');
check('exports renderPage12',            typeof local_pca_theta_pi.renderPage12 === 'function');

// -----------------------------------------------------------------------------
group('local_pca_theta_pi.js: state-aware wrappers (NEW round 5 step 10)');
const wrappers = [
...
for (const name of wrappers) {
  check(`exports ${name}`, typeof local_pca_theta_pi[name] === 'function');
}

// -----------------------------------------------------------------------------
group('local_pca_theta_pi.js: verbatim underscore-prefixed helpers (preserved from chat-33)');
const verbatim = [
...
// throw when called without a document.
{
  ...
  let threw1 = false;
  try { local_pca_theta_pi._refreshThetaPiLayerStatus({ layersPresent: new Set() }); }
  catch (e) { threw1 = true; }
  check('_refreshThetaPiLayerStatus tolerates no-document', !threw1);

  let threw2 = false;
  try { local_pca_theta_pi._refreshThetaPiPanelVisibility({ layersPresent: new Set() }); }
  catch (e) { threw2 = true; }
  check('_refreshThetaPiPanelVisibility tolerates no-document', !threw2);

  let threw3 = false;
  try { local_pca_theta_pi._drawThCusumHero({ data: null }); }
  catch (e) { threw3 = true; }
...
let threw = false;
try { local_pca_theta_pi.refreshThetaPiLayerStatus(); } catch (e) { threw = true; }
check('refreshThetaPiLayerStatus() with no state + null _pageState does not throw', !threw);

let threw2 = false;
try { local_pca_theta_pi.refreshThetaPiPanelVisibility(); } catch (e) { threw2 = true; }
