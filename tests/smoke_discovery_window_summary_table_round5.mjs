...
// smoke test for window_summary_table (per-window summary table — discovery stage).
//
...
const window_summary_table = await import(`${WORKSPACE}/atlases/inversion/pages/discovery/window_summary_table.js`);
const state = await import(`${WORKSPACE}/atlases/inversion/pages/discovery/window_summary_table/_state.js`);
...
check('window_summary_table has mount',                       typeof window_summary_table.mount === 'function');
check('window_summary_table has unmount',                     typeof window_summary_table.unmount === 'function');
...
try { await window_summary_table.mount(root, atlasState, registry); }
...
try { await window_summary_table.mount(root, atlasState2, registry); }
...
try { await window_summary_table.unmount(root); }
