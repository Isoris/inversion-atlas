...
//
// Sub-module + main re-export coverage for the boundary_refinement boundaries
// refinement page (verbatim chat-33 exports + lifecycle scaffolding).
//
// Round 5 step 22 (chat 39 cont., 2026-05-07): boundary_refinement promoted from
...
//
// All 4 chat-33 exports preserved verbatim — and unlike karyotype_tier (which
// needed manual AST shims into 2 of 4 helpers), boundary_refinement needed ZERO
...
//   - _bndKeyHandler short-circuits gracefully when boundary_refinement is not
//     active (no DOM in Node, so `document.getElementById('boundary_refinement')`
...
// `../inversion_review/boundary_refinement.js` (pre-migration path) and was never
// run by the harness.

import * as boundary_refinement from '../atlases/inversion/pages/review/boundary_refinement.js';
import * as state from '../atlases/inversion/pages/review/boundary_refinement/_state.js';

// Minimal global.document stub — boundary_refinement's renderBoundariesPage and
...
// -----------------------------------------------------------------------------
group('boundary_refinement.js: lifecycle entry-points');
check('exports mount',                          typeof boundary_refinement.mount === 'function');
check('exports unmount',                        typeof boundary_refinement.unmount === 'function');
check('exports refreshPage11 (wrapper)',        typeof boundary_refinement.refreshPage11 === 'function');
check('__MODULE_ID__ NOT exported',             !('__MODULE_ID__' in boundary_refinement));

// -----------------------------------------------------------------------------
group('boundary_refinement.js: 4 chat-33 exports preserved verbatim');
check('exports renderBoundariesPage',           typeof boundary_refinement.renderBoundariesPage === 'function');
check('exports _bndKeyHandler',                 typeof boundary_refinement._bndKeyHandler === 'function');
check('exports _bndAttachHotkeys',              typeof boundary_refinement._bndAttachHotkeys === 'function');
check('exports _bndDetachHotkeys',              typeof boundary_refinement._bndDetachHotkeys === 'function');
...
try { boundary_refinement.renderBoundariesPage(); }
catch (e) { r1OK = false; r1Err = e; }
check('renderBoundariesPage no-throw (slot=null path)',
      r1OK, r1Err ? r1Err.message : '');

// -----------------------------------------------------------------------------
group('_bndKeyHandler: short-circuits gracefully (boundary_refinement not active)');
// document.getElementById('boundary_refinement') → null → handler early-returns.
let r2OK = true; let r2Err = null;
try { boundary_refinement._bndKeyHandler({ key: 'a' }); }
catch (e) { r2OK = false; r2Err = e; }
check('_bndKeyHandler no-throw (boundary_refinement=null path)',
...
try { boundary_refinement._bndAttachHotkeys(); }
...
try { boundary_refinement._bndDetachHotkeys(); }
...
try { boundary_refinement.refreshPage11(synthState); }
