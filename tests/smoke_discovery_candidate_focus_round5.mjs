...
//
// Smoke test for the round-5-step-2 split of candidate_focus (chat 36, 2026-05-07).
// Mirrors smoke_discovery_page1_round4.mjs structure but for candidate_focus's
// candidate-detail deep-dive. Verifies:
//   1. candidate_focus.js + 5 sub-modules load (no parse-time cycle issue from
//      _wires.js → candidate_focus.js → _wires.js).
...
//      _pageState (the candidate_focus module's reference).
...
// ---------------------------------------------------------------------------
// Fake DOM (same as local_pca_dosage smoke; candidate_focus needs only a subset)
// ---------------------------------------------------------------------------
...
function makeTester(sel) {
  ...
  // (the tabBar query in refreshCandidateUI uses #tabBar button[data-page="candidate_focus"])
...
// candidate_focus reads / writes these elements during renderCandidateMetadata + wires.
const ROOT = new FakeElement('div', 'candidate_focus-root');
...
const candidate_focus = await import(`${WORKSPACE}/atlases/inversion/pages/discovery/candidate_focus.js`);
const page2State = await import(`${WORKSPACE}/atlases/inversion/pages/discovery/candidate_focus/_state.js`);
...
console.log('--- Smoke: candidate_focus module load ---');
check('candidate_focus default loads', typeof candidate_focus === 'object' && candidate_focus !== null);
check('candidate_focus exports mount', typeof candidate_focus.mount === 'function');
check('candidate_focus exports unmount', typeof candidate_focus.unmount === 'function');
check('candidate_focus exports renderCandidateMetadata', typeof candidate_focus.renderCandidateMetadata === 'function');
check('candidate_focus exports refreshCandidateUI', typeof candidate_focus.refreshCandidateUI === 'function');
check('candidate_focus exports _navigateToCandidate', typeof candidate_focus._navigateToCandidate === 'function');
check('candidate_focus exports wireCandidateNav', typeof candidate_focus.wireCandidateNav === 'function');
...
let mountOK = false;
try {
  await candidate_focus.mount(ROOT, atlasState, { resolve: async () => null });
  mountOK = true;
} catch (e) {
...
let okRender = true; try { candidate_focus.renderCandidateMetadata(directState); } catch (e) { okRender = false; console.log('  renderCandidateMetadata:', e.message); }
check('renderCandidateMetadata(state) ran without throwing', okRender);

let okRefresh = true; try { candidate_focus.refreshCandidateUI(directState); } catch (e) { okRefresh = false; console.log('  refreshCandidateUI:', e.message); }
check('refreshCandidateUI(state) ran without throwing', okRefresh);

let okNav = true; try { candidate_focus._navigateToCandidate(directState, synthCandidate); } catch (e) { okNav = false; console.log('  _navigateToCandidate:', e.message); }
check('_navigateToCandidate(state, c) ran without throwing', okNav);

let okWireNav = true; try { candidate_focus.wireCandidateNav(directState, synthCandidate); } catch (e) { okWireNav = false; console.log('  wireCandidateNav:', e.message); }
...
let unmountOK = false;
try {
  await candidate_focus.unmount(ROOT);
  unmountOK = true;
} catch (e) {
