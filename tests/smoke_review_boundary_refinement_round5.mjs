...
// unmount lifecycle smoke test for boundary_refinement (boundaries refinement
...
//     _pageState (matches karyotype_tier's stash pattern)
...
//   - _bndKeyHandler short-circuits when boundary_refinement element absent
//
// Cross-page guard-resolution audit performed step 22: NONE of
// boundary_refinement's 31 TODO_MISSING markers are already exported in the
// migrated tree. Confirms these are genuinely boundary_refinement-private
// helpers awaiting a Batch-2 extraction round.

const WORKSPACE = process.env.WORKSPACE || '/home/claude/workspace/atlas-workspace';
const boundary_refinement = await import(`${WORKSPACE}/atlases/inversion/pages/review/boundary_refinement.js`);
const state = await import(`${WORKSPACE}/atlases/inversion/pages/review/boundary_refinement/_state.js`);
...
// -----------------------------------------------------------------------------
// Minimal DOM polyfill — boundary_refinement uses getElementById, .style, .innerHTML,
...
check('boundary_refinement has mount',                    typeof boundary_refinement.mount === 'function');
check('boundary_refinement has unmount',                  typeof boundary_refinement.unmount === 'function');
check('boundary_refinement has refreshPage11 (wrapper)',  typeof boundary_refinement.refreshPage11 === 'function');
check('boundary_refinement has renderBoundariesPage',     typeof boundary_refinement.renderBoundariesPage === 'function');
check('boundary_refinement has _bndKeyHandler',           typeof boundary_refinement._bndKeyHandler === 'function');
check('boundary_refinement has _bndAttachHotkeys',        typeof boundary_refinement._bndAttachHotkeys === 'function');
check('boundary_refinement has _bndDetachHotkeys',        typeof boundary_refinement._bndDetachHotkeys === 'function');
...
// fresh.
try { boundary_refinement._bndDetachHotkeys(); } catch (_) {}
...
try { await boundary_refinement.mount(root, atlasState, registry); }
...
// -----------------------------------------------------------------------------
group('Smoke: _bndKeyHandler short-circuits when boundary_refinement element absent');
// FakeNode for 'boundary_refinement' exists but classList does not contain 'active'
// → handler early-returns at the active-class guard.
let kh1OK = true; let kh1Err = null;
try { boundary_refinement._bndKeyHandler({ key: 'a' }); }
catch (e) { kh1OK = false; kh1Err = e; }
check('_bndKeyHandler({key:"a"}) no-throw (boundary_refinement not active)',
      kh1OK, kh1Err ? kh1Err.message : '');

// Make boundary_refinement active and verify the handler still doesn't throw
...
const page11Node = _ensureNode('boundary_refinement');
page11Node.classList.add('active');
let kh2OK = true; let kh2Err = null;
try { boundary_refinement._bndKeyHandler({ key: 'z' /* unmatched */ }); }
...
try { boundary_refinement._bndKeyHandler({ key: 'a', ctrlKey: true, preventDefault: () => {} }); }
...
await boundary_refinement.unmount(root);
_resetNodes();
...
try { await boundary_refinement.mount(root, atlasState2, registry); }
...
try { boundary_refinement.refreshPage11(reRenderState); }
...
try { await boundary_refinement.unmount(root); }
...
try { await boundary_refinement.mount(root, atlasState3, registry); }
