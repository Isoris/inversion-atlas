...
// approach as page17/page18 (rounds 5 step 4-5): inject
// `const state = _pageState;` shim into every function body that reads
// bare `state`. 50 helpers got the shim.
//
// MIGRATION NOTE: page16 owns _csGetSyntenyBlocks (line 1488) and
// _csPermutationTest (line ~1791) — both previously runtime-guarded
// in page17 (synthesis stats profile). Round 5 step 5 noted these
// would land naturally with page16 migration; they're now exported
// at the bottom of this file so page17 can promote its runtime guards
...
//                        throughout page16 now resolve to the shared helper.
//
...
//                        these to imports would couple page16 to page1's
...
import { _pageState, _setActiveState } from './page16/_state.js';
...
// shim into the 28 bodies that read bare `state` (page17/18 pattern).
//
...
//      _csComputeSynteny are read by page17 (synthesis stats profile)
//      via runtime guards. Promoting these to imports in page17 is a
...
// Cross-page helpers (page17 reads these via runtime guards today)
export {
  _csGetSyntenyBlocks,
  _csPermutationTest,
  _csComputeSynteny,
...
/**
 * Mount: called by atlas_router when the user navigates to page16.
 *
 * Reads activeChrom from atlasState and builds a legacy-shape state
 * with the slots page16 reads: crossSpecies (the cs_breakpoints_v1
...
export async function mount(root, atlasState, registry) {
  ...
  catch (e) { console.warn('page16.mount: _renderCrossSpeciesPage threw —', e); }
