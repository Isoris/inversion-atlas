...
// Same AST-aware patcher approach as page16/page17/page18 (rounds 5 step 4-5
// + 11): inject `const state = _pageState;` shim into every function body
// that reads bare `state`. **39 of 64 helpers got the shim** (page16 was
// 28 of 50). **Direct twin of page16 in pattern shape** — closes the
// comparative group.
//
// MIGRATION NOTE: page16b owns six JSON layers (dotplot_mashmap_v1,
...
// other pages today — page16 reads page16b's `state.dotplotMashmap` as
...
// (unlike page16 → page17). Pure mechanical refactor.
...
//                        (same shared helper used by page16). 47 unguarded
//                        uses throughout page16b now resolve to the shared
//                        helper.
//
//  window.popgenDotplot  — KEPT as runtime guard. External vendor lib;
//                          typeof-checked at the single call site (line ~1351).
//                          Same disposition as page16's vendor guards.
//
...
import { _pageState, _setActiveState } from './page16b/_state.js';
...
// `state` (page16/page17/page18 pattern). The legacy `window.X = X`
...
/**
 * Mount: called by atlas_router when the user navigates to page16b.
 *
 * Reads activeChrom + activeCandidate from atlasState and builds a
 * legacy-shape state with the slots page16b reads:
 *   - state.crossSpecies (page16-owned, read-only here)
 *   - state.dotplotMashmap, state.syntenyMultispecies,
 *     state.phyloTree, state.dxyPerInversion, state.compTEFragility,
 *     state.karyotypeLineage (the six page16b-owned JSON layers)
 ...
 *     (read-only references from page16's territory)
 */
export async function mount(root, atlasState, registry) {
  ...
  catch (e) { console.warn('page16b.mount: _renderMultiSpeciesPage threw —', e); }
...
function _buildLegacyState(atlasState) {
  ...
  // Page16-owned slots that page16b reads
