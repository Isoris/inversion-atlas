...
**Pages:** cross_species_breakpoints (cross-species overview), multi_species_cockpit (multi-species cockpit), help (help)
...
`HANDOFF_BATCH_5.md` describes help as the **"Multi-species comparison page (Pairwise dotplots, synteny graph, breakpoint annotations)"** at lines 8159–9316. **This is wrong.**

In legacy/Inversion_atlas.html, lines 8159–9316 are the **static Quick-reference / Help page** (`<button data-page="help" data-stage="help">` at legacy line 5142).
...
The **actual** multi-species classification cockpit is **multi_species_cockpit** (`<div id="multi_species_cockpit">` at legacy line 8033, renderer `_renderMultiSpeciesPage` at legacy line 27497). I extracted multi_species_cockpit correctly per the line ranges in the handoff (8033–8100); the multi-species JS lives in `multi_species_cockpit.js`.

Decision: I followed the handoff's line ranges literally (8159–9316 → `help.html`) and made the JS module a stub with explanatory comments. The merge chat should treat help as a static help page.
...
|---|---:|---:|---|
| cross_species_breakpoints | 84 | 2,556 | `[20971..21114] ∪ [23717..26025] ∪ [28367..28419]` |
| multi_species_cockpit | 68 | 2,403 | `[26023..28366]` (incl. the two `DOTPLOT_MASHMAP_*` constants at 26023–26024 anchored as a prelude block) |
| help | 1,158 | 34 (stub) | none — static page |
...
**Owned by cross_species_breakpoints.js (cross-species page):**
...
1. **Where dotplot_mashmap_v1 IO lives.** The IO loader functions for `state.dotplotMashmap` (legacy 26026–26124) are co-located in `multi_species_cockpit.js` because the multi-species page is the primary consumer. Page16 reads `state.dotplotMashmap` only as an optional overlay for `_renderCrossSpeciesDotplot`. **The merge chat may decide to promote the IO loader to `shared/state_io.js` if both pages end up depending on it equally.**

2. **`_csBuildPermResultHtml` placement.** This function lives at legacy line 28367, *after* the entire multi-species block. It belongs logically to the cross-species permutation test (called from `_renderCrossSpeciesSynteny`'s click handler at legacy 25794). I extracted it into `cross_species_breakpoints.js` as Block C. The position in the legacy file appears to be an accident of patch ordering.

3. **`help.js` is a stub.** Because legacy help is purely declarative HTML with no render function, the JS module is a stub with `renderPage5(state)` (no-op) and a `PAGE5_META = { id, stage, label, num, static: true }` object. This keeps page-loader routing uniform.
...
├── cross_species_breakpoints.html       (   84 lines)
├── cross_species_breakpoints.js         (2,556 lines)
├── multi_species_cockpit.html      (   68 lines)
├── multi_species_cockpit.js        (2,403 lines)
├── help.html        (1,158 lines)
├── help.js          (   34 lines)
