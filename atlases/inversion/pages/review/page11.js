// =============================================================================
// inversion_review/page11.js — "boundaries" tab
// =============================================================================
...
// This is a stronger statement than page4 made (page4 needed shims into
// 2 of 4 helpers); page11 is even simpler than page4 in this dimension.
//
...
// ------------------------------------------------------------------
// pages.registry.json declares page11 has
...
// **The activeCandidate slot is correct** — page11 IS a candidate-level
...
// looks like page4's territory (page4 reads
// state.data.final_classification, the 14-axis classification), and
// `candidate_breeding_card` does not appear anywhere in page11's source
// at all. What page11 actually consumes (per the trailing comment
...
//
// **Notable correlate**: page4 (step 21 flag) declared `candidate_boundaries`
// which the page4 handoff said "looks like page11's territory" — yet
// page11 declares neither `candidate_boundaries` nor anything boundary-
// related. This is consistent with a SWAP pattern: page4's declared
// `candidate_sv_counts` + `candidate_boundaries` may have been swapped
// with page11's declared `candidate_final_class` + `candidate_breeding_card`.
...
// other three flags — page4, page6, page7).
//
...
// page11-private helpers awaiting a Batch-2 extraction round.)
// -----------------------------------------------------------------------------
//
...
/**
 * Mount: called by atlas_router when the user navigates to page11.
 *
 * Builds a legacy-shape state with the slots page11 reads (per the
...
export async function mount(root, atlasState, registry) {
  ...
  catch (e) { console.warn('page11.mount: renderBoundariesPage threw —', e); }
  ...
  catch (e) { console.warn('page11.mount: _bndAttachHotkeys threw —', e); }
...
function _buildLegacyState(atlasState) {
  ...
  // Transient slot — page11 reads state.data.final_classification (via
  // the same _bnd* helpers that page4 keys on). Default empty object so
