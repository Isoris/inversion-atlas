// pages/catalogue/confirmed_carousel/carousel.js
//
...
export function wireConfirmedCarouselNav(state) {
  ...
  _keyHandler = (evt) => {
    // Only act when confirmed_carousel is the visible page (#confirmed_carousel.active).
    // The atlas-router toggles .active on the page wrapper.
    const confirmed_carousel = document.getElementById('confirmed_carousel');
    if (!confirmed_carousel || !confirmed_carousel.classList
        || typeof confirmed_carousel.classList.contains !== 'function'
        || !confirmed_carousel.classList.contains('active')) return;
