...
//   `typeof X === 'function'`. Round 5 step 11 migrated page16 and made
//   them explicit ES exports (legacy lines ~1488 and ~1791 in legacy
//   single-file). They are now imported from '../comparative/page16.js'.
//   The mount() function bridges page17's legacyState into page16's
//   _pageState (same pattern as the existing page17 → page18 bridge),
//   so page16's verbatim helpers see live crossSpecies + candidateList
//   when page17 calls them during its render.
//
//   RESOLVED earlier: _esc imported from shared/page1_data_helpers.js;
//   _mpDeriveAutoPanel imported from sibling page18.js.
//
...
// (typeof X === 'function') to explicit imports now that page16 exports
// them. The mount() function bridges legacyState into page16's _pageState
// so these helpers see live data without page16 needing to be mounted.
import { _csGetSyntenyBlocks, _csPermutationTest } from '../comparative/page16.js';
import { _setActiveState as _setPage16State } from '../comparative/page16/_state.js';
...
function _spDeriveCsPermutation() {
  ...
  // imported from page16 (no more typeof guards). Both return null when
...
// Bridges state to page18 too so page17→page18 calls (_mpDeriveAutoPanel)
...
// sound — page17 doesn't mutate it, only page18 lazy-inits _markerPanel.
...
/**
 * Mount: called by atlas_router when the user navigates to page17.
 *
 * Reads activeChrom from atlasState and builds a legacy-shape state with
 * the slots page17 needs: candidateList, crossSpecies, _statsProfile
 * (lazy-init via _spEnsureState), _markerPanel (read-only — populated
 * by page18), data (chromosome precomp).
 *
 * Bridges legacyState into BOTH page18 and page16 so cross-page calls
 * see live data without those pages needing to be mounted independently.
 *   - page18: _mpDeriveAutoPanel (read-only).
 *   - page16: _csGetSyntenyBlocks + _csPermutationTest (read-only over
...
export async function mount(root, atlasState, registry) {
  ...
  // Bridge state to page18 so the cross-page call (_mpDeriveAutoPanel)
  // sees live data when called from inside page17's render.
  _setPage18State(legacyState);
  // Bridge state to page16 so the cross-page calls (_csGetSyntenyBlocks,
  ...
  catch (e) { console.warn('page17.mount: renderStatsProfilePage threw —', e); }

  if (atlasState.inversion) atlasState.inversion._page17State = legacyState;
}

/**
 * Unmount: clear page17 _pageState. Don't touch page18's or page16's
 ...
 * in the page17 → page{16,18} navigation flow.
 */
