// Atlas/inversion_catalogue/annotation_cockpit.js
// =============================================================================
// annotation_cockpit — Annotation cockpit (per-sample-lines canvas with cursor-driven
// candidate selection)
// (`<div id="annotation_cockpit">` contains canvas `annoCockpitCanvas` + footer panels)
//
// NOTE on handoff doc: HANDOFF_BATCH_3.md describes annotation_cockpit as "Manual
...
//
// External dependencies — kept as runtime `typeof X === 'function'` guards
// (NOT promoted to imports) until the owning pages migrate. Each call site
// already has a guard; missing deps gracefully degrade rather than throw:
//
...
//                                           per-candidate band purity for
//                                           tracked fish. Likely shared/
//                                           candidate.
//
//   global `state`                        — reads:
//                                             state.data
//                                             state.tracked
//                                             state.cockpitCursor (lazy-init)
//                                             state.cockpitSelectedBand
//                                             state._cockpitLastHapCandId
//                                             state.activeMode
//                                             state.candidates
...
function _ackEnsureState() {
  // Read from annotation_cockpit's _pageState (set by mount() or refreshAnnotationCockpit(state)).
  // Falls back to a transient empty object if neither has been called — keeps
  // pure helpers callable from tests without a mount.
...
//   - the user navigates to annotation_cockpit (tab click)
//   - candidates added/removed
//   - chromosome changes
function _refreshAnnotationCockpit() {
  _ackEnsureState();
  _annoCockpitInit();
  const items = _gatherActiveCandidatesForInheritance();
...
// Sets _pageState before delegating so all helpers (which read via
// _ackEnsureState → _pageState) see live data.
export function refreshAnnotationCockpit(state) {
  if (state) _setActiveState(state);
  return _refreshAnnotationCockpit();
}

// ---------------------------------------------------------------------------
// Atlas-router lifecycle (chat 36 round 5 step 6, 2026-05-07).
// ---------------------------------------------------------------------------

/**
 * Mount: called by atlas_router when the user navigates to annotation_cockpit.
 *
 * Reads activeChrom from atlasState and builds a legacy-shape state with
 * the slots annotation_cockpit needs: data (chromosome precomp with samples + windows
 * + chrom_len_bp), tracked (lasso/digit-key selection), cockpitCursor +
 * cockpitSelectedBand (lazy-init), candidates / candidates_detailed
...
function _buildLegacyState(atlasState) {
  ...
  legacy.tracked              = Array.isArray(inv.tracked) ? inv.tracked : [];
  legacy.cockpitCursor        = inv.cockpitCursor        || null;
  legacy.cockpitSelectedBand  = inv.cockpitSelectedBand  || null;
  legacy.candidates           = inv.candidates           || null;
  legacy.candidates_detailed  = inv.candidates_detailed  || null;
  legacy.activeMode           = inv.activeMode           || null;
