// pages/discovery/page_dosage_cluster.js
// =====================================================================
...
import { _pageState, _setActiveState } from './page_dosage_cluster/_state.js';
import {
  paintClusterCurves,
  findClusterAtPixel,
  verdictColor,
  verdictLabel,
  clusterColor,
} from './page_dosage_cluster/renderer.js';
import {
  createDosageClusterSelection,
  summariseKEntry,
  entryToDisplay,
  clusterSizesFromLabels,
} from './page_dosage_cluster/selection.js';
...
export async function mount(root, atlasState, registry) {
  ...
  catch (e) { console.warn('page_dosage_cluster.mount: refresh threw —', e); }

  try { initDosageClusterToolbar(); }
  catch (e) { console.warn('page_dosage_cluster.mount: toolbar wiring threw —', e); }

  if (atlasState.inversion) {
    atlasState.inversion._page_dosage_cluster_state = pageState;
  }
}

export async function unmount(root) {
  try { _teardownToolbar(_pageState); }
  catch (e) { console.warn('page_dosage_cluster.unmount: teardown threw —', e); }
