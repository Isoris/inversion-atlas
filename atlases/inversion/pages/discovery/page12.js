...
// markers because it didn't model lexical scope. Per the page1 round-2
...
// Every previously-flagged name is closure-scoped, declared as a local
// `const`/`let` inside the calling function. Same finding as page1
// round 2 (extractor didn't model lexical scope). The markers are
// deleted; bodies unchanged.
//   showHide  — local at line 97 (_refreshThetaPiPanelVisibility)
//   xToPx     — locals at lines 151, 460, 853 (per-renderer)
//   kColor    — local at line 155 (_drawThCusumHero)
//   q         — locals at lines 258, 304 (quantile lookups)
//   has       — local at line 86 (_refreshThetaPiPanelVisibility)
//   colorFor  — local at line 506 (_drawThLinesPanel)
//   palette   — local at line 617 (_drawThSimMatPanel)
//   xAt       — let at line 706 (_drawThZPanel)
//   yAt       — local at line 744 (_drawThZPanel)
//   yToPx     — local at line 461 (_drawThLinesPanel)
//   toX, toY  — locals at lines 947-948 (_drawThPcaPanel)
//   fillFor   — local at line 988 (_drawThPcaPanel)
// ---------------------------------------------------------------------------
...
// trio that gives atlas_router a uniform handle for page12.
// ---------------------------------------------------------------------------
...
/**
 * Convenience: render every θπ panel at once. Mirrors what page1's
...
export function renderPage12(state) {
  ...
  try { _refreshThetaPiLayerStatus(s); }       catch (e) { console.warn('page12.renderPage12: _refreshThetaPiLayerStatus —', e); }
  try { _refreshThetaPiPanelVisibility(s); }   catch (e) { console.warn('page12.renderPage12: _refreshThetaPiPanelVisibility —', e); }
  try { _drawThCusumHero(s); }                 catch (e) { console.warn('page12.renderPage12: _drawThCusumHero —', e); }
  try { _drawThLinesPanel(s); }                catch (e) { console.warn('page12.renderPage12: _drawThLinesPanel —', e); }
  try { _drawThSimMatPanel(s); }               catch (e) { console.warn('page12.renderPage12: _drawThSimMatPanel —', e); }
  try { _drawThZPanel(s); }                    catch (e) { console.warn('page12.renderPage12: _drawThZPanel —', e); }
  try { _drawThAnchorStripPanel(s); }          catch (e) { console.warn('page12.renderPage12: _drawThAnchorStripPanel —', e); }
  try { _drawThPcaPanel(s); }                  catch (e) { console.warn('page12.renderPage12: _drawThPcaPanel —', e); }
}

/**
 * Mount: called by atlas_router when the user navigates to page12.
 *
 * Reads activeChrom from atlasState and builds a legacy-shape state
 * with the slots page12 reads: data (chromosome precomp containing
...
export async function mount(root, atlasState, registry) {
  ...
  catch (e) { console.warn('page12.mount: renderPage12 threw —', e); }
