...
// Mirrors page12's _refreshThetaPiPanelVisibility: hides the empty-state
...
export function renderGhslPanels(state) {
  ...
  try { _renderGhslCtrlBar(s); } catch (e) { console.warn('page15 ctrlBar:', e); }
  try { _drawGhslMeanStrip(s); } catch (e) { console.warn('page15 meanStrip:', e); }
  try { _drawGhslHeatmap(s); }   catch (e) { console.warn('page15 heatmap:', e); }
  try { _drawGhslLines(s); }     catch (e) { console.warn('page15 lines:', e); }
  try { _renderGhslSampleTable(s); } catch (e) { console.warn('page15 sampleTable:', e); }
}
...
export async function mount(root, atlasState, registry) {
  ...
  // stays visible and the panel slots stay hidden — same pattern as page12.
  try { refreshGhslPanelVisibility(legacyState); }
  catch (e) { console.warn('page15.mount: refreshGhslPanelVisibility threw —', e); }

  try { renderGhslPanels(legacyState); }
  catch (e) { console.warn('page15.mount: renderGhslPanels threw —', e); }
  ...
  if (typeof document !== 'undefined') {
    ...
    if (sel && !sel.__page15Wired) {
      sel.addEventListener('change', () => {
        try { renderGhslPanels(legacyState); }
        catch (e) { console.warn('page15 scaleSelect change:', e); }
      });
