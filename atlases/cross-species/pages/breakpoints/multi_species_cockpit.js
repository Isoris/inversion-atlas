// multi_species_cockpit.js — Multi-species classification cockpit (multi_species_cockpit)
...
// the multi-species page is the primary consumer; cross_species_breakpoints reads state.dotplotMashmap
...
//
// Round 5 step 20 (chat 39 cont., 2026-05-07): refactored from chat-33
// "0 explicit ES exports + window-expose globals + bare-state references"
// pattern to atlas-router-compatible mount/unmount + _pageState live-binding.
// Same AST-aware patcher approach as cross_species_breakpoints/stats_profile/marker_readiness (rounds 5 step 4-5
// + 11): inject `const state = _pageState;` shim into every function body
// that reads bare `state`. **39 of 64 helpers got the shim** (cross_species_breakpoints was
// 28 of 50). **Direct twin of cross_species_breakpoints in pattern shape** — closes the
// comparative group.
//
// MIGRATION NOTE: multi_species_cockpit owns six JSON layers (dotplot_mashmap_v1,
// synteny_multispecies_v1, phylo_tree_v1, dxy_per_inversion_v1,
// comparative_te_breakpoint_fragility_v1, karyotype_lineage_v1) and the
// classifications LS layer. None of its helpers are runtime-guarded from
// other pages today — cross_species_breakpoints reads multi_species_cockpit's `state.dotplotMashmap` as
// an optional overlay slot only (not via function call). So no
// cross-page guard-resolution payoff round is unblocked by this migration
// (unlike cross_species_breakpoints → stats_profile). Pure mechanical refactor.

//
// =============================================================================
// TODO_MISSING markers — RESOLVED 2026-05-07 round 5 step 20.
// =============================================================================
//
//  _esc                — RESOLVED via import from shared/page1_data_helpers.js
//                        (same shared helper used by cross_species_breakpoints). 47 unguarded
//                        uses throughout multi_species_cockpit now resolve to the shared
//                        helper.
//
//  window.popgenDotplot  — KEPT as runtime guard. External vendor lib;
//                          typeof-checked at the single call site (line ~1351).
//                          Same disposition as cross_species_breakpoints's vendor guards.
//
//  window.popgenFocalVsBg — KEPT as runtime guard. External vendor lib;
//                           typeof-checked at the single call site (line ~1429).
//                           Same disposition as cross_species_breakpoints's vendor guards.
//
// Top-level state slots this module reads/writes (managed by shared/state.js):
//   READS: state.crossSpecies (cross_species_breakpoints-owned), state._crossSpeciesUI.active_id,
...
//
// EXTERNAL STATE READS (confirmed during step 20 audit — flagged here for
// the merge chat to ensure the global `state` object exposes these slots):
//   state.crossSpecies               — written by cross_species_breakpoints.js (_storeCrossSpecies)
...
// =============================================================================
// turn 121: multi-species classification cockpit (multi_species_cockpit)
// -----------------------------------------------------------------------------
// Reads:
...
function _msRenderActiveHeader() {
  ...
  if (!bp) {
    slot.innerHTML = '<span class="ms-active-header-empty">' +
                     ...
                     '<a href="#" class="ms-jump-link" data-jump="cross_species_breakpoints">switch to page 13 \u2192</a>' +
                     '</span>';
  ...
  slot.innerHTML =
    ...
    '<a href="#" class="ms-jump-link" data-jump="cross_species_breakpoints">view on page 13 \u2192</a>';
  const link = slot.querySelector('.ms-jump-link');
  if (link) {
    link.addEventListener('click', (ev) => {
      ev.preventDefault();
...
function _renderMultiSpeciesPage() {
  ...
  if (link && !link._msWired) {
    ...
    link.addEventListener('click', (ev) => {
      ev.preventDefault();
      const btn = document.querySelector('#tabBar button[data-page="help"]');
      if (btn) {
        btn.click();
        setTimeout(() => {
...
// =============================================================================
// turn 123: comparative TE breakpoint fragility (multi_species_cockpit right column + center)
// -----------------------------------------------------------------------------
...
// =============================================================================
// turn 124: karyotype-context lineage layer (multi_species_cockpit chromosome-scale evidence)
// -----------------------------------------------------------------------------
...
// ===========================================================================
// Atlas-router lifecycle (chat 39 cont. round 5 step 20, 2026-05-07).
//
// Explicit ES exports for the public surface of the multi-species
// classification cockpit page. The verbatim helper bodies above are
// unchanged from chat-33 except for the AST-injected
// `const state = _pageState;` shim into the 39 bodies that read bare
// `state` (cross_species_breakpoints/stats_profile/marker_readiness pattern). The legacy `window.X = X`
// expose blocks are kept in place for backward-compat with any direct
// global callers; the explicit ES exports below are additive.
//
// Public entries fall into six logical groups:
//   1. Constants — the per-layer LS keys + the default species manifest.
//   2. Render entry points — _renderMultiSpeciesPage is the main one,
//      with _msRenderActiveHeader/TreeSvg/CenterColumn/DetailColumn as
//      column-level handles.
//   3. IO loaders — six JSON layer loaders, each a 5-tuple
//      (_isXJSON, _storeX, _persistX, _restoreX, _clearX): dotplot
//      mashmap, synteny multispecies, phylo tree, dxy per inversion,
//      comparative TE fragility, karyotype lineage.
//   4. MS state/init helpers — _msInitState, _msPersistUI,
//      classifications-related (init, persist, set, get, clear,
//      buildTSV, downloadTSV).
//   5. MS analysis helpers — getActiveBreakpoint, lineage
//      distribution, auto-suggest architecture/age, dxy/TE-fragility
//      lookups, synteny-blocks-for-species, newick parser,
//      HTML builders.
//   6. Karyotype helpers — getKaryotypeEntry (focal/Cgar chr),
//      polarizeKaryotypeEvent (+ refinement variant), refinement
...
// IO loaders — dotplot_mashmap_v1
export {
  _isDotplotMashmapJSON,
  _storeDotplotMashmap,
  _persistDotplotMashmap,
  _restoreDotplotMashmap,
  _clearDotplotMashmap,
};

// IO loaders — synteny_multispecies_v1
export {
  _isSyntenyMultispeciesJSON,
  _storeSyntenyMultispecies,
  _persistSyntenyMultispecies,
  _restoreSyntenyMultispecies,
  _clearSyntenyMultispecies,
};

// IO loaders — phylo_tree_v1
export {
  _isPhyloTreeJSON,
  _storePhyloTree,
  _persistPhyloTree,
...
// IO loaders — dxy_per_inversion_v1
export {
  _isDxyPerInversionJSON,
  _storeDxyPerInversion,
  _persistDxyPerInversion,
  _restoreDxyPerInversion,
  _clearDxyPerInversion,
};

// IO loaders — comparative_te_breakpoint_fragility_v1
export {
  _isCompTEFragilityJSON,
  _storeCompTEFragility,
  _persistCompTEFragility,
  _restoreCompTEFragility,
  _clearCompTEFragility,
};

// IO loaders — karyotype_lineage_v1
export {
  _isKaryotypeLineageJSON,
  _storeKaryotypeLineage,
  _persistKaryotypeLineage,
  _restoreKaryotypeLineage,
  _clearKaryotypeLineage,
};

// MS state / init / classifications
export {
  _msInitState,
  _msPersistUI,
  _msInitClassifications,
  _msPersistClassifications,
  _msSetClassification,
  _msGetClassification,
  _msClearClassification,
  _msBuildClassificationTSV,
  _msDownloadClassificationTSV,
};

// MS analysis helpers
export {
  _msGetActiveBreakpoint,
  _msGetEffectiveSpeciesList,
  _msGetLineageDistribution,
  _msAutoSuggestArchitecture,
  _msAutoSuggestAgeModel,
  _msGetDxyForBreakpoint,
  _msGetTEFragilityForBreakpoint,
  _msGetSyntenyBlocksForSpecies,
...
// Karyotype helpers (chromosome-scale lineage layer)
export {
  ...
  _msGetEffectiveTargetsForCell,
  _msSummarizeRefinement,
  _msAdjustConfidenceForRefinement,
};

/**
 * Public entry — state-aware wrapper around _renderMultiSpeciesPage.
 ...
 */
export function renderMultiSpeciesPage(state) {
 ...
 *
 * Reads activeChrom + activeCandidate from atlasState and builds a
 * legacy-shape state with the slots multi_species_cockpit reads:
 *   - state.crossSpecies (cross_species_breakpoints-owned, read-only here)
 *   - state.dotplotMashmap, state.syntenyMultispecies,
 *     state.phyloTree, state.dxyPerInversion, state.compTEFragility,
 *     state.karyotypeLineage (the six multi_species_cockpit-owned JSON layers)
 *   - state._multiSpeciesUI, state._msClassifications,
 *     state.classifications (UI + classifications scratch)
 *   - state._crossSpeciesUI (read-only: active_id used to find the
 *     focused breakpoint context)
 *   - state.repeatDensity, state._csDotplotPanel*, state._focalVsBg
 *     (read-only references from cross_species_breakpoints's territory)
 */
export async function mount(root, atlasState, registry) {
  const legacyState = _buildLegacyState(atlasState);
  _setActiveState(legacyState);

  try { _renderMultiSpeciesPage(); }
  catch (e) { console.warn('multi_species_cockpit.mount: _renderMultiSpeciesPage threw —', e); }

  if (atlasState.inversion) atlasState.inversion._page16bState = legacyState;
}
...
function _buildLegacyState(atlasState) {
  ...
  // Page16-owned slots that multi_species_cockpit reads
  legacy.crossSpecies     = inv.crossSpecies     || null;
  legacy.repeatDensity    = inv.repeatDensity    || null;
  legacy._crossSpeciesUI  = inv._crossSpeciesUI  || null;
  legacy._csDotplotPanel    = inv._csDotplotPanel    || null;
  legacy._csDotplotPanelFp  = inv._csDotplotPanelFp  || null;
  legacy._focalVsBg         = inv._focalVsBg         || null;

  // Page16b-owned data slots — six JSON layers
  legacy.dotplotMashmap     = inv.dotplotMashmap     || null;
  legacy.syntenyMultispecies= inv.syntenyMultispecies|| null;
  legacy.phyloTree          = inv.phyloTree          || null;
  legacy.dxyPerInversion    = inv.dxyPerInversion    || null;
  legacy.compTEFragility    = inv.compTEFragility    || null;
  legacy.karyotypeLineage   = inv.karyotypeLineage   || null;

  // Page16b-owned UI scratch + classifications
  legacy._multiSpeciesUI    = inv._multiSpeciesUI    || null;
  legacy._msClassifications = inv._msClassifications || null;
  legacy.classifications    = inv.classifications    || null;
