// cross_species_breakpoints.js — Cross-species breakpoints page (cross-species comparative dashboard)
...
//
// Round 5 step 11 (chat 36, 2026-05-07): refactored from chat-33
// "0 exports, bare-state references" pattern to atlas-router-compatible
// mount/unmount + _pageState live-binding. Same AST-aware patcher
// approach as stats_profile/marker_readiness (rounds 5 step 4-5): inject
// `const state = _pageState;` shim into every function body that reads
// bare `state`. 50 helpers got the shim.
//
// MIGRATION NOTE: cross_species_breakpoints owns _csGetSyntenyBlocks (line 1488) and
// _csPermutationTest (line ~1791) — both previously runtime-guarded
// in stats_profile (synthesis stats profile). Round 5 step 5 noted these
// would land naturally with cross_species_breakpoints migration; they're now exported
// at the bottom of this file so stats_profile can promote its runtime guards
// to imports in a follow-up round.

//
// =============================================================================
// TODO_MISSING markers — RESOLVED 2026-05-07 round 5 step 11.
// =============================================================================
//
...
//                        (optional hook).
//
//  window.popgenDotplot, window.popgenFocalVsBg — KEPT as runtime guards.
//                        External vendor libs; typeof-checked at every
//                        call site.
//
...
// =============================================================================
// =============================================================================
// v4 turn 74 — Cross-species breakpoints page (cross_species_breakpoints)
// -----------------------------------------------------------------------------
...
// Keyboard wiring — only acts when cross_species_breakpoints is the active page.
function _wireCrossSpeciesKeys() {
  const state = _pageState;
  if (state._crossSpeciesKeysBound) return;
  document.addEventListener('keydown', (e) => {
    const page = document.getElementById('cross_species_breakpoints');
...
// =============================================================================
// v4 turn 75 — Cross-species synteny module (cross_species_breakpoints, sub-section)
// -----------------------------------------------------------------------------
// Macro-synteny interpretation layer on top of the cs_breakpoints_v1 v2 data.
// Renders inside the focus column of cross_species_breakpoints, BELOW the per-breakpoint focus
// view. Components:
...
function _renderCrossSpeciesFocalVsBg() {
  ...
  try {
    panel = window.popgenFocalVsBg.makeFocalVsBgPanel({
      page_id:           'cross_species_breakpoints',
      anchor_id:         bp.id,
...
// multi_species_cockpit.js — cross_species_breakpoints only reads state.dotplotMashmap as an optional overlay.
...
// ===========================================================================
// Atlas-router lifecycle (chat 36 round 5 step 11, 2026-05-07).
//
// Explicit ES exports for the public surface of the cross-species
// breakpoints page. The verbatim helper bodies above are unchanged
// from chat-33 except for the AST-injected `const state = _pageState;`
// shim into the 28 bodies that read bare `state` (stats_profile/18 pattern).
//
// Public entries fall into four groups:
//   1. Render entry points — _renderCrossSpeciesPage is the main one,
//      with _renderCrossSpeciesToolbar/Catalogue/Focus/Synteny/Dotplot/
//      FocalVsBg as panel-level handles.
//   2. Cross-page helpers — _csGetSyntenyBlocks + _csPermutationTest +
//      _csComputeSynteny are read by stats_profile (synthesis stats profile)
//      via runtime guards. Promoting these to imports in stats_profile is a
//      follow-up round task.
//   3. IO helpers — _storeCrossSpecies, _clearCrossSpecies, etc., are
//      called by the data-loader pipeline (loadMultipleJSONs in the
//      legacy host).
//   4. Constants — CROSS_SPECIES_TOOL, CS_EVENT_DEF are referenced by
//      the JSON dispatch chain.
// ===========================================================================
...
export {
  ...
  _renderCrossSpeciesCatalogue,
  _renderCrossSpeciesFocus,
  _renderCrossSpeciesSynteny,
  _renderCrossSpeciesDotplot,
  _renderCrossSpeciesFocalVsBg,
};

// Cross-page helpers (stats_profile reads these via runtime guards today)
export {
  _csGetSyntenyBlocks,
  _csPermutationTest,
  _csComputeSynteny,
  _csSyntenyEdgesByChrom,
  _csInversionContexts,
  _csBuildPermResultHtml,
};

// IO helpers (called by data-loader pipeline)
export {
  _isCrossSpeciesJSON,
  _storeCrossSpecies,
  _persistCrossSpecies,
  _restoreCrossSpecies,
  _clearCrossSpecies,
};
...
/**
 * Public entry — state-aware wrapper around _renderCrossSpeciesPage.
 ...
 */
export function renderCrossSpeciesPage(state) {
 ...
 * with the slots cross_species_breakpoints reads: crossSpecies (the cs_breakpoints_v1
 * data), repeatDensity (TEfull JSONs from boundaries page),
 * candidateList, dotplotMashmap (optional overlay), data (chrom
 * precomp), cur (current window index), and all the cs* caches +
 * UI flags cross_species_breakpoints owns.
 */
export async function mount(root, atlasState, registry) {
  const legacyState = _buildLegacyState(atlasState);
  _setActiveState(legacyState);

  try { _renderCrossSpeciesPage(); }
  catch (e) { console.warn('cross_species_breakpoints.mount: _renderCrossSpeciesPage threw —', e); }

  if (atlasState.inversion) atlasState.inversion._page16State = legacyState;
}
...
function _buildLegacyState(atlasState) {
  ...
  // Chromosome precomp data
  const chrom = sh.activeChrom;
  legacy.data = (chrom && inv.tracks && inv.tracks[chrom]) ? inv.tracks[chrom] : null;

  // Page16-owned caches + UI scratch (preserve identity across mounts
  // so subsequent renders can hit the cache; null-default if missing).
  legacy._crossSpeciesUI            = inv._crossSpeciesUI            || { active_id: null, sort: 'pos', filter: { search: '' } };
  legacy._csSyntenyCache            = inv._csSyntenyCache            || null;
  legacy._csSyntenyEdgesCache       = inv._csSyntenyEdgesCache       || null;
  legacy._csInversionContextCache   = inv._csInversionContextCache   || null;
  legacy._csOverlayIndex            = inv._csOverlayIndex            || null;
  ...
  legacy._csHoverActive             = inv._csHoverActive             || null;
  legacy._csHoverRaf                = inv._csHoverRaf                || null;
  legacy._csHeaderScrollWired       = inv._csHeaderScrollWired       || false;
  legacy._crossSpeciesKeysBound     = inv._crossSpeciesKeysBound     || false;
