// =============================================================================
// inversion_review/ancestry_per_window.js — "9 ancestry" tab
// =============================================================================
// Stage:        review (per-window ancestry view)
// Legacy DOM:   <div id="ancestry_per_window"> (legacy lines 7660–7670)
...
// canvas-track helpers shared between popstats and ancestry_per_window into a sibling
// module under inversion_review/.
//
// Decision (BATCH_2_NOTES): same lifecycle-wrapper pattern as popstats.js.
//
// Registry mismatch flagged for Quentin
// -------------------------------------
// pages.registry.json declares ancestry_per_window has
//   "requires_layers": ["candidate_marker_primers"]
//   "requires_slots":  ["activeCandidate"]
// but the ancestry_per_window.js header (and the legacy DOM contract) clearly describe
// ancestry_per_window as a CHROMOSOME-LEVEL ancestry view driven by a
...
export function showAncestryPage(state) {
  ...
  if (typeof fn === 'function') {
    try {
      ...
      console.warn('[ancestry_per_window] renderAncestryPage threw:', err);
    }
    return;
  }
 ...
 * (mirrors confirmed_carousel/local_pca_ghsl/help wrapper pattern). The chat-33
...
/**
 * Mount: called by atlas_router when the user navigates to ancestry_per_window.
 *
 * Builds a legacy-shape state with the slots ancestry_per_window will eventually need
