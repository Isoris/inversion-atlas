...
The shared module `atlases/inversion/shared/xpehh_per_window.js` ships
the following pure helpers; they consume an `xpehh_per_window.json`
payload matching §"Layer schema" above. The popstats page-6 renderer
plugs into these:

| Export | Purpose |
|---|---|
| `XPEHH_PER_WINDOW_TOOL` / `XPEHH_PER_WINDOW_LS_KEY` / `XPEHH_OUTLIER_Z_DEFAULT` (2.0) / `XPEHH_OUTLIER_PCT_DEFAULT` (0.01) | Schema-identifier + threshold constants. |
| `isXpehhPerWindowJSON(data)` | Schema validator → `{ok, reasons}`. |
| `storeXpehhPerWindow(state, parsed)` / `persistXpehhPerWindow(state)` / `restoreXpehhPerWindow(state)` / `clearXpehhPerWindow(state)` | LS lifecycle matching dxy_per_inversion / cross_species. |
| `xpehhWindowsForChrom(state, chrom)` | Per-chrom arrays bundle (`{window_start_bp, window_end_bp, xpehh_mean, ...}`) or `null`. |
| `xpehhValueAtPosition(state, chrom, pos_bp)` | Lookup the window whose `[start_bp, end_bp]` contains `pos_bp`; returns `{idx, start_bp, end_bp, xpehh_mean, norm_xpehh_mean, n_snps}` or `null`. |
| `xpehhValuesInRange(state, chrom, start_bp, end_bp)` | Slice the per-window arrays for a bp range. |
| `xpehhOutliers(state, chrom, opts)` | Windows where `|norm_xpehh_mean| ≥ z_threshold` (defaults to 2.0) OR top `pct` by magnitude. |
| `xpehhTrackHeader(state)` | `'test_cohort_id vs ref_cohort_id (n_test/n_ref)'` for the popstats panel title (spec §Tests: "test/ref cohort labels visible in the track title"). |
| `xpehhAlignsWithTrack(state, otherChrom, otherWindows)` | Track-grid alignment check (spec §Tests: "window grid matches F_ST exactly"). |

These primitives are headless-tolerant (no DOM, no fetch) and never
inject into globals; they are consumed by the popstats page-6
renderer once the layer JSON lands.
